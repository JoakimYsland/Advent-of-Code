...
for y, row in enumerate(input_map): 
	for x, c in enumerate(row): 
		...
		if len(path) > 0: 
			path_lengths_from_a.append(len(path))

end_time_ms = round(time.time() * 1000)

print("Time:", end_time_ms - start_time_ms)

path_lengths_from_a.sort()
shortest_path = path_lengths_from_a[0] - 1 # Starting position does not count as a step
num_paths = len(path_lengths_from_a)
